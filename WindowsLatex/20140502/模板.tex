...
\newfontfamily\song{AR PL UMing CN} %宋体，这个应该是系统默认有的
\newfontfamily\hei{AR PL UKai CN} %黑体，这个应该是系统默认有的
...
\maketitle
\begin{chabstract}
\hei 本文讨论了本科层次的泛函分析教材中函数空间和数列空间的实例，\ldots.\\
\end{chabstract}
...
{\xiaosi{\song{在一般的本科生泛函分析教材中，\ldots 如~$L^p(\mathbf{E})$~和~$l^p$~}}}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%以上为论文引言部分%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
一切满足
~\begin{math}(\sum\limits^{\infty}_{i=1}|\xi_i|^p)^{1/p}<+\infty\end{math}~    %注意命令 \limits
...
\begin{displaymath}
\lim_{n\to\infty}\sum_{k=1}^n \frac{1}{k^2}= \frac{x^2}{6}
\end{displaymath}
